cordova.define("cordova-plugin-http.CordovaHttpPlugin", function (require, exports, module) {
    ...
    var http = {
        ...
        post: function (url, params, headers, timeout, success, failure) {
            headers = mergeHeaders(this.headers, headers);
            return exec(success, failure, "CordovaHttpPlugin", "post", [url, params, headers, timeout]);
        },
        postJson: function (url, json, headers, timeout, success, failure) {
            try {
                var serializedBody = JSON.stringify(json);

                if (headers) {
                    var objectKeys = Object.keys(headers);
                    var headersWithoutContentType = objectKeys.map(function (item) {
                    ...
                    headersWithoutContentType['Content-Type'] = 'application/json';
                    headers = headersWithoutContentType
                } else {
                    headers = { 'Content-Type': 'application/json' }
                }
                return exec(success, failure, "CordovaHttpPlugin", "post", [url, serializedBody, headers, timeout]);
            } catch (ex) {
                console.error("ERRORE: ", ex);
            }

        },
        get: function (url, params, headers, timeout, success, failure) {
            headers = mergeHeaders(this.headers, headers);
            return exec(success, failure, "CordovaHttpPlugin", "get", [url, params, headers, timeout]);
        },
        head: function (url, params, headers, timeout, success, failure) {
            headers = mergeHeaders(this.headers, headers);
            return exec(success, failure, "CordovaHttpPlugin", "head", [url, params, headers, timeout]);
        },
        uploadFile: function (url, params, headers, filePath, name, timeout, success, failure) {
            headers = mergeHeaders(this.headers, headers);
            return exec(success, failure, "CordovaHttpPlugin", "uploadFile", [url, params, headers, filePath, name, timeout]);
        },
